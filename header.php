...
if(isset($_GET["id"])){
$query='UPDATE msg SET seen_status=1 where msg.sender='.$_GET["id"].' AND msg.reciever='.$_SESSION["id"];
$result=mysqli_query($conn,$query);
}
//for login user
$query='select * from user where user.id='.$_SESSION['id'].' order by id DESC LIMIT 1';
$last_seen=mysqli_query($conn,$query);
$last_seen=mysqli_fetch_all($last_seen,MYSQLI_ASSOC);
//where pre means privious  post means current datatime
$pre_datetime=explode(' ',$last_seen[0]['last_seen']);
$post_date_time=explode(' ',date('Y-m-d H:i:s'));

if(strtotime($pre_datetime[0])<strtotime($post_date_time[0]))
{
    $last_seen=date('D M d h:i A',strtotime($pre_datetime[0]));
}
elseif(strtotime($pre_datetime[0])==strtotime($post_date_time[0]))
{
    if(strtotime($pre_datetime[1])<strtotime($post_date_time[1]))
    {
        $pre_time=explode(':',$pre_datetime[1]);
        $post_time=explode(':',$post_date_time[1]);
        $pre_h=$pre_time[0];
        $pre_m=$pre_time[1];
        $post_h=$post_time[0];
        $post_m=$post_time[1];
        ...
        $actual_m=$post_m-$pre_m;
       
        $actual_h=$post_h-$pre_h;
        if($actual_m>60)
        {
            $actual_h++;
            $actual_m=(int)$actual_m-60;
        }
        if($actual_h==0){
            $actual_h='';
        }
        ...
        $last_seen=$actual_h. $actual_m.'ago';
    }
}
//for login user
if(isset($_GET['id'])){
$query='select * from user where user.id='.$_GET['id'].' order by id DESC LIMIT 1';
$last_seen2=mysqli_query($conn,$query);
$last_seen2=mysqli_fetch_all($last_seen2,MYSQLI_ASSOC);
//where pre means privious  post means current datatime
$pre_datetime=explode(' ',$last_seen2[0]['last_seen']);
$post_date_time=explode(' ',date('Y-m-d H:i:s'));

if(strtotime($pre_datetime[0])<strtotime($post_date_time[0]))
{
    $last_seen2=date('D M d h:i A',strtotime($pre_datetime[0]));
}
elseif(strtotime($pre_datetime[0])==strtotime($post_date_time[0]))
{
    if(strtotime($pre_datetime[1])<strtotime($post_date_time[1]))
    {
        ...
        $last_seen2=$actual_h. $actual_m.'ago';
    }
}
}
...
$query='SELECT * FROM `user` JOIN msg on user.id=msg.reciever WHERE (msg.reciever='.$_SESSION["id"].' AND msg.seen_status=0)';
$notification=mysqli_query($conn,$query);
$notification=mysqli_fetch_all($notification,MYSQLI_ASSOC);
                    ...
                    <a class="nav-link active py-0 py-lg-8" id="tab-chats" href="#tab-content-chats" title="Chats"
                        ...
                        <div class="icon icon-xl icon-badged">
                            ...
                            <div class="badge badge-circle bg-primary">
                                <span><?php echo count($notification);?></span>
                            </div>
                                            ...
                                            <div class="card border-0 mt-5">
                                                <div class="card-body">
                                                    ...
                                                    <div class="row align-items-center gx-5">
                                                        <div class="col-auto">
                                                            <div
                                                                class="avatar <?php if($_SESSION['status']==1){ echo " avatar-online "; }else{ echo " avatar-offline ";}?>">
                                                        ...
                                                        <div class="col">
                                                            ...
                                                            <p><?php if($_SESSION['status']==1){ echo "online"; }else{ echo "offline"; }?>
                                                            </p>
                                                        </div>
                                                        <div class="col-auto">
                                                            <div class="form-check">
                                    ...
                                    <a href="massage.chat.php?id=<?php echo $value['id']?>"
                                        ...
                                        <div class="card-body">
                                            <div class="row gx-5">
                                                ...
                                                <?php 
                                                         $query='SELECT * FROM `user` JOIN msg on user.id=msg.reciever WHERE (msg.reciever='.$_SESSION["id"].' AND msg.seen_status=0 AND msg.sender='.$value['id'].') order By msg.id DESC LIMIT 1';
                                                        //  echo $query;
                                                          $notification_sub_time=mysqli_query($conn,$query);
                                                         $notification_sub_time=mysqli_fetch_all($notification_sub_time,MYSQLI_ASSOC);
                                                ?>
                                                <div class="col">
                                                    <div class="d-flex align-items-center mb-3">
                                                        <h5 class="me-auto mb-0"><?php echo ucwords($value['user_name']); ?></h5>
                                                        <span class="text-muted extra-small ms-2">
                                                            <?php  if(isset($notification_sub_time[0]['datetime']))
                                                         {
                                                            echo date('h:i A',strtotime($notification_sub_time[0]['datetime']));
                                                         }?>
                                                        </span>
                                                    </div>
                                                    ...
                                                    <div class="d-flex align-items-center">
                                                        ...
                                                        <?php 
                                                         $query='SELECT * FROM `user` JOIN msg on user.id=msg.reciever WHERE (msg.reciever='.$_SESSION["id"].' AND msg.seen_status=0 AND msg.sender='.$value['id'].')';
                                                         $notification_sub=mysqli_query($conn,$query);
                                                         $notification_sub=mysqli_fetch_all($notification_sub,MYSQLI_ASSOC);
                                                         ?>
                                                        <?php if(count($notification_sub)!=0){?>
                                                        <div class="badge badge-circle bg-primary ms-5">
                                                            
                                                            <span><?php if(isset($notification_sub)){ echo count($notification_sub); }?></span>
                                                        </div>
                                                        <?php }?>
                                                    </div>
                                                </div>
                                ...
                                <div class="card border-0">
                                    <div class="card-body">
                                        <div class="row align-items-center gx-5">
                                            <div class="col-auto">
                                                <div class="avatar">
                                                    <img src="<?php echo $_SESSION['profile']; ?>" alt="#" class="avatar-img">
